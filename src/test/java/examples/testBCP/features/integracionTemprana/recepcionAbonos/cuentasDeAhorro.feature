...
Feature: Recepción de abonos en cuenta de Ahorro
  ...
  Background:
    ...
    * def TEST_013 = read('../../../data/jsonData/resources/TEST013.json')
    * def TEST_010 = read('../../../data/jsonData/resources/TEST010.json')
    * def TEST_137 = read('../../../data/jsonData/resources/TEST137.json')

# TEST_013
  Scenario: En dólares como tercero con RUC
    # Endpoint mockeado
    Given path 'achoperations', 'initiate', 'mock'
    And request TEST_013
    When method POST
    Then status 200
...
#TEST_137
  Scenario: Desde BIM hacia YAPE por QR como tercero con DNI
    # Endpoint mockeado
    Given path 'achoperations', 'initiate', 'mock'
    And request TEST_137
    When method POST
    Then status 200
    And match response == dataAV2
    * def AV2 = response
